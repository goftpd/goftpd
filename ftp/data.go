...
func (s *Server) newPassiveDataConn(ctx context.Context, dataProtected bool) (*passiveDataConn, error) {
	...
	for {
		...
		if dataProtected {
			ln, err = tls.Listen("tcp", fmt.Sprintf("%s:%d", s.PublicIP, port), s.tlsConfig)
		} else {
			ln, err = net.Listen("tcp", fmt.Sprintf("%s:%d", s.PublicIP, port))
		}
...
func (d *passiveDataConn) String() string {
	p1 := d.port / 256
	p2 := d.port - (p1 * 256)
